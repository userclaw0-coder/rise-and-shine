...
export function getPlannerRefinementEventsInRange(userId, startDateStr, endDateStr) {
  ...
  return wrap(
    supabase
      ...
      .select("id, task_id, event_type, created_at, value")
      .eq("user_id", userId)
      .or("event_type.eq.planner_refinement_accepted,event_type.eq.planner_refinement_dismissed,event_type.eq.planner_refinement_applied,event_type.eq.updated")
      .gte("created_at", start.toISOString())
      .lt("created_at", end.toISOString())
      .order("created_at", { ascending: false })
  );
