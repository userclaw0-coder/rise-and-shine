...
export async function updateTemplateOrder(items) {
  // items: [{id, sort_order}] — update only sort_order so we don't overwrite other columns
...
function localDateToUtcRange(dateStr, timezone) {
  ...
  if (tzY < y || (tzY === y && tzM < m) || (tzY === y && tzM === m && tzD < d)) {
    tNoon = new Date(Date.UTC(y, m - 1, d, 24, 0, 0, 0));
...
export function getCompletedEventsInRange(userId, startDateStr, endDateStr) {
  ...
  const { end: endExclusive } = localDateToUtcRange(endDateStr, TIMEZONE);
  return wrap(
    supabase
      .from("task_events")
