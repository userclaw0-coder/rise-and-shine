...
export async function persistMessages(userId, messages) {
  const rows = messages.map((m) => ({
    ...
    client_request_id: m.client_request_id || null,
    scope: null,
  }));
...
/**
 * Look up a recent user message by client_request_id (idempotency check).
 * Returns the user message row plus the most recent assistant message that
 * followed it (if any), or null if nothing was found in the lookback window.
 */
export async function findRecentRequest(userId, clientRequestId, lookbackMinutes = 5) {
  ...
  // Find the most recent assistant message after that user row. We pick the
  // latest (not earliest) because an interrupted attempt may have persisted a
  // stub like "(I didn't finish — error: ...)"; if a later attempt for the
  // same client_request_id succeeded with real content, we want that one.
  const { data: assistantRow } = await supabase
    .from("chat_messages")
    .select("id, content")
    .eq("user_id", userId)
    .eq("role", "assistant")
    .gte("created_at", userRow.created_at)
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  return { userRow, assistantRow: assistantRow || null };
}
