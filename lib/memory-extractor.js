...
async function pullRecentSignal(userId, since) {
  ...
  const [chatRes, eventRes, catRes, profileRes, notesRes, dailyNotesRes] = await Promise.all([
    supabaseAdmin
      .from("chat_messages")
    ...
    supabaseAdmin
      ...
      .eq("user_id", userId)
      .maybeSingle(),
    // User-curated notes (title+body, often prefixed [session]/[handoff]/<project>):
    // these capture context the user explicitly wants the system to know.
    supabaseAdmin
      .from("notes")
      .select("title, body, jarvis_feed, created_at, updated_at")
      .eq("user_id", userId)
      .or(`created_at.gte.${sinceIso},updated_at.gte.${sinceIso}`)
      .order("created_at", { ascending: false })
      .limit(50),
    // Reflective daily journal entries — often contain family/life context
    // that would otherwise never make it into the warm-tier memory store.
    supabaseAdmin
      .from("daily_notes")
      .select("date, note, created_at, updated_at")
      .eq("user_id", userId)
      .or(`created_at.gte.${sinceIso},updated_at.gte.${sinceIso}`)
  ...
  // Filter out empty daily-note rows — the UI auto-creates empty stubs.
  const dailyNotes = (dailyNotesRes.data || []).filter(
...
function formatSignalForPrompt({
  ...
  if (notes.length > 0) {
    parts.push(`\n# User-curated notes in window (${notes.length})`);
    parts.push(`(High-signal context the user explicitly wrote down. Titles may use prefixes like [session], [handoff], or project names. Extract durable facts liberally — these are intentional captures, not casual chat.)`);
    let charBudget = 6000;
    for (const n of notes) {
      const ts = (n.updated_at || n.created_at || "").slice(0, 16);
      const flag = n.jarvis_feed ? " · jarvis_feed" : "";
      const head = `\n[${ts}${flag}] ${n.title || "(untitled)"}\n`;
      const body = (n.body || "").slice(0, 1500);
      const block = `${head}${body}`;
      if (charBudget - block.length < 0) {
        parts.push(`\n… (${notes.length - (parts.length || 0)} more notes truncated)`);
        break;
      }
      parts.push(block);
  ...
  if (dailyNotes.length > 0) {
    parts.push(`\n# Daily journal entries in window (${dailyNotes.length})`);
    parts.push(`(Reflective journal — often contains family / life context that should become durable memories.)`);
    let charBudget = 4000;
...
export async function extractMemoriesForUser(
  ...
  const signalCount =
    signal.chatMessages.length +
    signal.taskEvents.length +
    (signal.notes?.length || 0) +
    (signal.dailyNotes?.length || 0);
