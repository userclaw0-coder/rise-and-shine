...
import { getPageCoachDigest } from "./jarvis-context";
...
export async function buildSystemPrompt(userId) {
  return Promise.race([
    buildSystemPromptInner(userId),
    new Promise((resolve) =>
      setTimeout(() => resolve(PERSONA), PROMPT_TIMEOUT_MS)
    ),
  ]);
}

async function buildSystemPromptInner(userId) {
    ...
    try {
      ...
      if (digest && digest.length > 0) {
        ...
        for (const d of digest) {
          ...
          contextParts.push(
            `- ${d.scope} (${d.turns} turns, last ${when}): user asked "${d.last_user_question || "…"}"; coach replied "${d.last_coach_reply || "…"}"`
          );
        }
      }
    } catch {
