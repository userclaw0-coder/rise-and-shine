...
import { outcomeProgress, outcomesProgress } from "./iscProgress.js";
...
- **Projects**: Use get_project_details and get_project_kb to understand a project's full context before giving advice. Use update_project_kb when the user shares project info (contacts, reference numbers, specs, dates). Use add_project_resource to track links and documents. When the user describes a new project or situation, use create_project to set it up, then create tasks as high-level phases.
...
- **Capture context as it happens** (don't wait for "goodbye"). The user's notes table is how future sessions remember this one — and chats sometimes get cut off mid-flight, so saving early matters:
  - Use \`save_session_summary\` whenever you've done meaningful work or learned non-trivial context in a turn — completed a block of tasks, walked through a project's state, decided on a plan, or the user shared family/life/project background. Don't wait for the user to say "thanks". A summary every ~5-10 substantive turns is better than none.
  - Use \`create_note\` for standalone durable context: a project orientation dump, a handoff prompt, a long explanation the user wrote (title prefix \`[handoff]\`, \`[<project name>]\`, or \`<Project> — <topic>\`). Save this immediately when it appears, not at session end.
  - For atomic facts (single contacts, dates, preferences), \`write_memory\` is fine — but anything multi-paragraph belongs in notes so future agents can read the full context.
  - Skip the meta-narration ("I'll save this for you") — just call the tool.
...
async function buildSystemPromptInner(userId, opts = {}) {
  ...
  try {
    ...
    if (profile) {
      const allOutcomes = profile.desired_outcomes || [];
      ...
      // ISC (Ideal State Criteria) progress — concrete verification items per outcome
      const overall = outcomesProgress(allOutcomes);
      if (overall.total > 0) {
        contextParts.push(
          `ISC progress: ${overall.met}/${overall.total} criteria met across ${overall.outcome_count_with_criteria} outcomes (${overall.percent}%)`
        );
        const withCriteria = allOutcomes
          .filter((o) => Array.isArray(o.criteria) && o.criteria.length > 0)
          .slice(0, 6);
        for (const o of withCriteria) {
          const p = outcomeProgress(o);
          contextParts.push(
            `  • ${o.title.slice(0, 60)}: ${p.met}/${p.total} (${p.percent}%)`
          );
        }
      }
      ...
      // Working-style traits: user-declared patterns that should shape how
      // Jarvis (and any sub-coach) breaks down tasks and frames suggestions.
      // Each trait carries a coach_rule string that we surface verbatim so
      // the model has explicit guidance, not just an abstract label.
      const traits = profile.working_style?.traits;
      if (Array.isArray(traits) && traits.length > 0) {
        contextParts.push("Working-style traits (LOAD-BEARING — shape every suggestion):");
        for (const t of traits) {
          if (!t || !t.label) continue;
          const line = `  • ${t.label}: ${t.description || ""}`.trim();
          contextParts.push(line);
          if (t.coach_rule) {
            contextParts.push(`    → Rule: ${t.coach_rule}`);
          }
        }
      }
    }
    ...
    if (projectsRes.length > 0) {
      ...
      for (const p of projectsRes) {
        ...
        if (p.hasKB) line += " [KB]";
        if (p.driveFolderUrl) line += " [Drive]";
        contextParts.push(line);
      }
    }
  ...
  try {
    ...
    for (const ws of workspaces || []) {
      wsMap[ws.category_id] = {
        ...
        driveFolderUrl: ws.workspace?.drive_folder_url || "",
      };
    }
    ...
    const projects = categories.map((cat) => {
      ...
      return {
        ...
        driveFolderUrl: ws.driveFolderUrl || "",
        weight,
      };
    });
