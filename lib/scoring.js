...
import { OCCAM_WORKOUTS } from "./occam.js";
import { getWorkoutPlanForSchedule } from "./occamSchedule.js";
...
export function computeTaskScore(task, options) {
  const {
    ...
    // PR-C: vector + activation-energy inputs (both optional)
    outcomeIscState = null, // Map<outcome_id, {unmetCount, totalCount}>
    // PR-D: morning state + reflection-derived AE adjustments (both optional)
    morningEnergy = null,   // "low" | "medium" | "high"
    aeAdjustments = null,   // Map<vector_key, number>  (+ = hard, - = easy)
  } = options || {};
  ...
  // PR-C: ISC pull. Tasks tagged to outcomes with stalled (unmet) ISCs
  // pull harder, so the day's vector progress gets nudged into still-
  // unfinished outcomes rather than ones that are already crushed.
  let iscPullBonus = 0;
  if (
    outcomeIscState &&
    outcomeIscState.get &&
    Array.isArray(task.outcome_ids) &&
    task.outcome_ids.length > 0
  ) {
    let unmetSum = 0;
    for (const id of task.outcome_ids) {
      const state = outcomeIscState.get(String(id));
      if (state && state.totalCount > 0) unmetSum += state.unmetCount;
    }
    if (unmetSum > 0) iscPullBonus = Math.min(Math.round(unmetSum * 1.5), 12);
  }

  // PR-C: activation-energy bonus. Reward task shapes that make GETTING
  // STARTED easy — Tom's biggest blocker. Terminal time-boxes ("Do 30 min
  // on X") get the strongest pull because they have zero decisions to
  // make. Logistics-first precursors (GATHER/FIND/ORGANIZE) get next-
  // strongest because they remove blockers for the doing-work. Short
  // efforts get a small bump.
  let activationEnergyBonus = 0;
  if (task.is_terminal === true) {
    activationEnergyBonus += 8;
  }
  const titlePrefixed = /^(\s*)(GATHER|FIND|ORGANIZE)\s*:/i.test(
    String(task.title || "")
  );
  if (
    titlePrefixed ||
    hasAnyTag(task, ["logistics-first", "gather", "find", "organize"])
  ) {
    activationEnergyBonus += 5;
  }
  const aeEff = hoursEffort(task);
  if (aeEff != null && aeEff <= 0.25) {
    activationEnergyBonus += 3;
  }
  if (activationEnergyBonus > 10) activationEnergyBonus = 10;

  // PR-D: morning energy bias. Low energy tilts the day toward smaller,
  // lower-AE work; high energy lets bigger / higher-leverage tasks
  // surface. The signal is intentionally modest — morning state nudges,
  // it doesn't dominate.
  let morningEnergyBias = 0;
  if (morningEnergy === "low") {
    const eMins = (hoursEffort(task) || 0) * 60;
    if (eMins > 0 && eMins <= 20) morningEnergyBias += 4;
    if (task.is_terminal) morningEnergyBias += 3;
    if (eMins > 60) morningEnergyBias -= 6;
  } else if (morningEnergy === "high") {
    if (
      hasAnyTag(task, ["high-leverage", "high leverage", "high_leverage"])
    ) {
      morningEnergyBias += 4;
    }
  }

  // PR-D: reflection-derived AE adjustment. Patterns that have repeatedly
  // felt hard get a small penalty; patterns that felt easy get a small
  // boost. Applied at the vector level so individual tasks inherit their
  // pattern's track record.
  let aeAdjustment = 0;
  if (aeAdjustments && aeAdjustments.get) {
    if (Array.isArray(task.outcome_ids)) {
      for (const id of task.outcome_ids) {
        const v = aeAdjustments.get(`outcome:${id}`);
        if (typeof v === "number") aeAdjustment -= v;
      }
    }
    if (task.primary_life_domain) {
      const v = aeAdjustments.get(`domain:${task.primary_life_domain}`);
      if (typeof v === "number") aeAdjustment -= v;
    }
    if (task.category_id) {
      const v = aeAdjustments.get(`category:${task.category_id}`);
      if (typeof v === "number") aeAdjustment -= v;
    }
    if (aeAdjustment > 6) aeAdjustment = 6;
    if (aeAdjustment < -6) aeAdjustment = -6;
  }
...
export function buildRationale(task, scoringResult, mode) {
  ...
  // PR-C factors: vector + activation energy.
  if (task.is_terminal) {
    parts.push("Bite-size next session — zero decisions to start");
  } else {
    const titlePrefixed = /^(\s*)(GATHER|FIND|ORGANIZE)\s*:/i.test(
      String(task.title || "")
    );
    if (titlePrefixed) {
      parts.push("Logistics-first precursor — clears the path");
    }
  }
  if ((c.iscPullBonus || 0) >= 6) {
    parts.push("Advances multiple stalled ISCs");
  } else if ((c.iscPullBonus || 0) > 0) {
    parts.push("Advances a stalled ISC");
  }
