...
export function buildRationale(task, scoringResult, mode) {
  ...
  if (c.isQuickWin) {
    ...
    parts.push(
      eff != null
        ? `Quick win (${Math.round(eff * 60)} min)`
        : "Quick win"
    );
  }
  ...
  if (catName && (c.categoryComponent || 0) > 16) {
    const modeNote = c.modeAdjustment > 0
      ? ` boosted by ${mode} mode`
      : "";
    parts.push(`Strong ${catName} category fit${modeNote}`);
  }
  ...
  if ((c.stalenessComponent || 0) >= 2) {
    parts.push("Hasn\u2019t been touched recently");
  }
  ...
  if (task.due_date) {
    ...
    if (daysUntil <= 2 && daysUntil >= 0) {
      parts.push(daysUntil === 0 ? "Due today" : `Due in ${daysUntil} day${daysUntil > 1 ? "s" : ""}`);
    }
  }

  if (task.parent_task_id && (c.subtaskComponent || 0) > 0) {
    parts.push("Subtask of a larger goal");
  }

  if (parts.length === 0) {
    return "Best available fit for your current focus";
  }
