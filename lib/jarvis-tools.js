...
import { writeMemory, searchMemories } from "./memories.js";
...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    name: "update_task",
    description:
      "Update an existing task's properties. Use get_task_details or get_backlog first to find the task_id. Use `phase` to move a task between time-window buckets (immediate / this_week / next_2w / next_30d / ongoing / blocked / someday); pass null to clear phase.",
    input_schema: {
      type: "object",
      properties: {
...
async function execCreateTask(params, userId) {
  ...
  const { data: task, error } = await supabase
    .from("tasks")
    .insert(row)
    .select("id, title, status, priority, effort_hours, due_date, category_id, phase")
    .single();
  if (error) throw error;
...
async function execUpdateTask(params, userId) {
  ...
  if (Object.keys(updates).length > 0) {
    const { data: task, error } = await supabase
      ...
      .eq("id", params.task_id)
      .eq("user_id", userId)
      .select("id, title, status, priority, effort_hours, due_date, phase")
      .single();
    if (error) throw error;
  ...
  const { data: updated, error: fetchErr } = await supabase
    .from("tasks")
    .select("id, title, status, priority, effort_hours, due_date, phase, tags:task_tags(tag:tags(name))")
    .eq("id", params.task_id)
    .single();
...
async function execAnalyzeProjectPlan(params, userId) {
  ...
  const { data: tasks } = await supabase
    .from("tasks")
    .select("id, title, status, priority, effort_hours, due_date, parent_task_id, tags:task_tags(tag:tags(name)), outcome_ids, primary_life_domain, created_at")
    .eq("user_id", userId)
    .eq("category_id", catId)
    .order("created_at");
