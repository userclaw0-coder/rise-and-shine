...
import { maybeDecomposeTask } from "./autoDecompose.js";
import {
  writeMemory,
  searchMemories,
  updateMemory,
...
const TOOL_DEFINITIONS = [
    ...
    input_schema: {
      ...
      properties: {
        ...
        priority: {
          ...
          enum: ["Critical", "High", "Medium", "Low"],
          description:
            "Priority level. Default Medium. Mapping: Critical = drop-everything / 'urgent' / 'asap' / 'right now'; High = important this week; Medium = normal backlog; Low = nice-to-have.",
        },
        effort_hours: {
          type: "number",
        ...
        due_date: {
          type: "string",
          description:
            "Due date in YYYY-MM-DD format. SET THIS whenever the user signals urgency — phrases like 'urgent', 'today', 'asap', 'right now', 'do today', 'before EOD' → due_date = today; 'tomorrow' → today + 1; 'this week' → end of current week; 'by Friday' / explicit dates → that date. Without a due_date the Eisenhower matrix on /backlog can't classify the task as urgent, so leaving this null when the user said 'urgent' is a bug. If there is genuinely no deadline (e.g. 'someday I want to'), leave null.",
        },
        tags: {
          type: "array",
          items: { type: "string" },
  ...
  },
  {
    name: "decompose_task",
    description:
      "Run the auto-decompose pipeline on one task right now. Loads the task + project context (mantra, KB), asks the LLM to emit ordered ≤30-min next-right-actions optimized for low activation energy, and writes them as subtasks under the parent. Idempotent — skips if the task is small enough, already decomposed, terminal, or already has open children. Use this manually when the user asks to break something down outside the automatic completion-driven path.",
    input_schema: {
      type: "object",
      properties: {
        task_id: {
          type: "string",
          description: "UUID of the parent task to decompose.",
        },
      },
      required: ["task_id"],
    },
...
async function execCompleteTask(params, userId) {
  ...
  // Auto-refill the project's next_action if this task was it. If the
  // refill picked a chunky task, fire-and-forget auto-decompose so the
  // next page load surfaces a ≤30-min leaf instead of an intimidating
  // parent. Never block completion on the LLM call.
  const refill = await autoRefillAfterCompletion(userId, params.task_id);
  const refillTaskId = refill?.next_action?.task_id;
  if (refillTaskId && refill?.next_action?.needs_breakdown) {
    maybeDecomposeTask(userId, refillTaskId).catch(() => {});
  }
...
async function execDecomposeTask(params, userId) {
  const result = await maybeDecomposeTask(userId, String(params.task_id || ""));
  return { ok: !!result.decomposed, ...result };
}
...
const EXECUTORS = {
  ...
  decompose_task: execDecomposeTask,
  ...
  update_project_ws: execUpdateProjectWorkspace,
  get_project_kb: execGetProjectKnowledge,
  update_project_kb: execUpdateProjectKnowledge,
  add_project_resource: execAddProjectResource,
  recent_imports: execGetRecentImportSummary,
  ...
  // Recurring task templates
  create_recurring: execCreateRecurringTemplate,
  update_recurring: execUpdateRecurringTemplate,
  archive_recurring: execArchiveRecurringTemplate,
  list_recurring: execListRecurringTemplates,
