...
function hasWorkspacePatch(patch) {
  ...
  return Boolean(
    cleanText(patch.mantra, 240) ||
      ...
      cleanText(patch.knowledge_base_append, 8000)
  );
}
...
function normalizeWorkspacePatch(workspacePatch) {
  ...
  const kb_append = cleanText(workspacePatch?.knowledge_base_append, 8000);
  if (kb_append) normalized.knowledge_base_append = kb_append;
...
export function buildProjectJsonSeed({
  ...
  return {
    ...
    project: {
      ...
      knowledge_base: cleanText(workspace?.knowledge_base, 12000),
    },
    vision: {
    ...
    ordering: {
      task_order_ids: Array.isArray(taskOrderIds) ? taskOrderIds.map(String) : [],
      subtask_order_ids: typeof subtaskOrderIds === "object" ? subtaskOrderIds : {},
    },
  };
}
...
Required JSON schema:
{
  ...
  "workspace_patch": {
    ...
    "resources": [{ "label": "string", "url": "string", "kind": "folder|doc|ai|archive|other", "status": "active|pending|expired|reference", "notes": "optional context" }],
    "health_needs": {
      "relationships": 0,
      "financial": 0,
      "wellbeing": 0,
    ...
    "knowledge_base_append": "optional string — appended to existing knowledge base"
  },
  "task_actions": [
    {
      "id": "string",
      "action": "update_task|create_root_task|create_subtask|archive_task|deprioritize_task|reorder_tasks|reorder_subtasks|set_dependency",
      "task_id": "required for update/archive/deprioritize/set_dependency",
      "parent_task_id": "required for create_subtask and reorder_subtasks",
      "blocked_by_task_id": "required for set_dependency — the task that blocks this one",
      ...
      "task_order_ids": ["for reorder_tasks: ordered array of existing root task ids"],
      "subtask_order_ids": ["for reorder_subtasks: ordered array of existing subtask ids"]
    }
  ],
  "alignment_actions": [
    {
  ...
  "vision_suggestions": [
    {
      ...
      "outcome": {
        "id": "optional string",
        "title": "string",
        "domain": "optional human need strategy key",
  ...
  ensureArray(raw?.task_actions)
    ...
    .forEach((item, idx) => {
      ...
      if (action === "reorder_tasks") {
        const orderIds = ensureArray(item?.task_order_ids).map((id) => String(id)).filter(Boolean);
      ...
      if (action === "reorder_subtasks") {
        const subIds = ensureArray(item?.subtask_order_ids).map((id) => String(id)).filter(Boolean);
        if (subIds.length === 0 || !parentTaskId) return;
        normalized.task_actions.push({
          id: cleanText(item?.id, 80) || `task_action_${idx + 1}`,
          action,
          ...
          title,
          summary,
          subtask_order_ids: subIds,
        });
        return;
      }

      if (action === "set_dependency") {
        const blockedBy = cleanText(item?.blocked_by_task_id, 80);
        if (!taskId || !blockedBy) return;
        normalized.task_actions.push({
          id: cleanText(item?.id, 80) || `task_action_${idx + 1}`,
          action,
          task_id: taskId,
...
export function flattenExternalProjectImportActions(normalized, categoryId = null) {
  ...
  ensureArray(normalized?.task_actions).forEach((item) => {
    ...
    if (item.action === "reorder_tasks") {
      actions.push({
        ...base,
        category_id: String(categoryId || normalized?.meta?.project_category_id || ""),
        apply_patch: {
          reorder_tasks: item.task_order_ids || [],
        },
      });
      return;
    }
    if (item.action === "reorder_subtasks") {
      actions.push({
        ...base,
        parent_task_id: cleanText(item?.parent_task_id, 80),
        category_id: String(categoryId || normalized?.meta?.project_category_id || ""),
        apply_patch: {
          reorder_subtasks: item.subtask_order_ids || [],
        },
      });
      return;
    }
    if (item.action === "set_dependency") {
      actions.push({
        ...base,
        task_id: cleanText(item?.task_id, 80),
        apply_patch: {
          set_dependency: true,
          blocked_by_task_id: cleanText(item?.blocked_by_task_id, 80),
        },
      });
    }
  });
