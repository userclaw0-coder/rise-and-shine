import { mergeProjectWorkspace } from "./projectWorkspace.js";
import { computeTaskScore } from "./scoring.js";
import { countRefinementActions } from "./planner-refinement-events.js";
import {
  HUMAN_NEED_STRATEGY_KEYS as LIFE_DOMAIN_KEYS,
  getHumanNeedStrategyLabel,
} from "./humanNeedStrategies.js";
...
function inferLifeDomainFromText(task, profile) {
  ...
  const candidates = [
    { key: "business", terms: ["certainty", "business", "sales", "revenue", "client", "product", "automation", "predictable", "stable"] },
    { key: "finances", terms: ["variety", "finance", "money", "cash", "rental", "house", "property", "budget", "tax", "novelty"] },
    { key: "health", terms: ["connection", "health", "workout", "sleep", "meal", "body", "recovery", "family", "friend", "relationship"] },
    { key: "relationships", terms: ["growth", "learn", "study", "read", "course", "skill", "practice", "improve"] },
    { key: "lifestyle", terms: ["contribution", "serve", "help", "mentor", "give", "support", "publish", "share"] },
    { key: "growth", terms: ["significance", "recognition", "mastery", "status", "impact", "prestige", "win"] },
  ];
  ...
  return {
    ...
    signals: {
      ...
      top_worked_domains: topWorkedDomains.map((row) => ({
        ...row,
        label: getHumanNeedStrategyLabel(row.key),
      })),
...
function defaultAlignmentFix(task, profile) {
  ...
  return {
    ...
    summary: "This task is active but not linked to a desired outcome or human need strategy.",
    apply_patch: {
      task: {
