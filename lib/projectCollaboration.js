...
import { defaultProjectWorkspace, mergeProjectWorkspace } from "./projectWorkspace.js";
...
export async function ensureSharedProjectWorkspace(categoryId) {
  ...
  if (existing) {
    return {
      ...
      subtask_order_ids: existing.subtask_order_ids || {},
      knowledge_base: existing.knowledge_base || "",
    };
  }
  ...
  return {
    ...
    subtask_order_ids: created.subtask_order_ids || {},
    knowledge_base: created.knowledge_base || "",
  };
}
...
async function fetchMemberRows(categoryIds) {
  return chunkedIn(
    "project_memberships",
    "category_id, user_id, role, email_snapshot",
    "category_id",
    categoryIds
  );
}
...
// Supabase REST encodes `.in()` filters into the URL query string. At ~37
// chars per UUID, batches over ~200 IDs blow past the gateway URL limit and
// surface as a bare `TypeError: fetch failed`. Chunk at 100 to stay well
// under all known gateway / proxy limits.
const IN_FILTER_CHUNK_SIZE = 100;

async function chunkedIn(table, selectCols, column, values) {
  if (!Array.isArray(values) || values.length === 0) return [];
  const out = [];
  for (let i = 0; i < values.length; i += IN_FILTER_CHUNK_SIZE) {
    const slice = values.slice(i, i + IN_FILTER_CHUNK_SIZE);
    const { data, error } = await serviceSupabase
      .from(table)
      .select(selectCols)
      .in(column, slice);
...
async function loadAssignmentsForTasks(taskIds) {
  return chunkedIn(
    "task_assignments",
    "task_id, user_id, email_snapshot, assigned_by, created_at",
    "task_id",
    taskIds
  );
}
...
export async function getProjectPageData(userId, categoryId) {
  ...
  return {
    ...
    knowledge_base: workspaceRow.knowledge_base || "",
    subtask_order_ids: workspaceRow.subtask_order_ids || {},
...
export async function saveSharedProjectWorkspace(userId, categoryId, patch = {}) {
  ...
  const nextWorkspace = normalizeWorkspacePayload(
    {
      ...
      ...(Array.isArray(patch.outcome_ids)
        ? { outcome_ids: patch.outcome_ids.map((v) => String(v)).filter(Boolean) }
        : {}),
      ...(Array.isArray(patch.life_domains)
        ? { life_domains: patch.life_domains.map((v) => String(v)).filter(Boolean).slice(0, 3) }
        : {}),
      ...(patch.primary_life_domain !== undefined
        ? { primary_life_domain: patch.primary_life_domain || null }
        : {}),
      ...(patch.last_aligned_at !== undefined
        ? { last_aligned_at: patch.last_aligned_at === "now" ? new Date().toISOString() : patch.last_aligned_at || null }
        : {}),
      ...(patch.next_action !== undefined
        ? { next_action: patch.next_action }
        : {}),
      ...(patch.drive_folder_url !== undefined
        ? { drive_folder_url: patch.drive_folder_url || null }
        : {}),
    },
  ...
  const payload = {
    ...
    knowledge_base: patch.knowledge_base !== undefined ? String(patch.knowledge_base || "") : (current.knowledge_base || ""),
    task_order_ids:
    ...
    subtask_order_ids:
      patch.subtask_order_ids !== undefined
        ? (typeof patch.subtask_order_ids === "object" ? patch.subtask_order_ids : {})
        : (current.subtask_order_ids || {}),
    updated_at: new Date().toISOString(),
  };
  const { data, error } = await serviceSupabase
    .from("shared_project_workspaces")
