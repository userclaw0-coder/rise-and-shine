 ...
 * embedding-provider rate limits if the array is large — caller should chunk if >50).
 */
...
/**
 * Update fields on an existing memory. If `content` is provided, the embedding
 * is regenerated so semantic search stays accurate. Other fields update in
 * place. Pass scope_id="" to clear it (global memories).
 */
export async function updateMemory(userId, memoryId, fields, { client = supabaseAdmin } = {}) {
  if (!userId) throw new Error("userId required");
  if (!memoryId) throw new Error("memoryId required");
  const updates = {};
  if (fields.content !== undefined) {
    const trimmed = String(fields.content).trim();
    if (!trimmed) throw new Error("content must be a non-empty string");
    updates.content = trimmed;
    updates.embedding = vectorLiteral(await embed(trimmed));
  }
  if (fields.kind !== undefined) {
    ...
    if (!allowedKinds.includes(fields.kind))
      throw new Error(`kind must be one of ${allowedKinds.join("|")}`);
    updates.kind = fields.kind;
  }
  if (fields.importance !== undefined) {
    const imp = Number(fields.importance);
    if (!Number.isFinite(imp) || imp < 1 || imp > 10)
      throw new Error("importance must be an integer 1-10");
    updates.importance = imp;
  }
  if (fields.scope_type !== undefined) {
    const allowedScopes = ["global","outcome","project","task","person"];
    if (!allowedScopes.includes(fields.scope_type))
      throw new Error(`scope_type must be one of ${allowedScopes.join("|")}`);
    updates.scope_type = fields.scope_type;
  }
  if (fields.scope_id !== undefined) {
    updates.scope_id = fields.scope_id || null;
  }
  if (Object.keys(updates).length === 0) {
    // no fields changed — return current row
    const { data, error } = await client
      .from("memories")
      .select("id, scope_type, scope_id, kind, content, importance")
      .eq("id", memoryId)
      .eq("user_id", userId)
      .maybeSingle();
    if (error) throw new Error(`updateMemory lookup failed: ${error.message}`);
...
/**
 * Hard delete a memory by id. For soft archive (preferred — keeps audit
 * trail and is reversible), use `archive()` instead.
 */
export async function deleteMemory(userId, memoryId, { client = supabaseAdmin } = {}) {
  if (!userId) throw new Error("userId required");
  if (!memoryId) throw new Error("memoryId required");
  const { error } = await client
    .from("memories")
