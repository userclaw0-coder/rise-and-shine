-- Task–outcome and task–life-domain alignment (for analytics)
...
-- Primary life domain: one of the vision life_domains keys (business, finances, health, relationships, lifestyle, growth)
alter table public.tasks
  add column if not exists primary_life_domain text;

-- Optional: multiple life domains this task touches
alter table public.tasks
...
comment on column public.tasks.primary_life_domain is 'Primary life domain key from vision (e.g. business, health)';
comment on column public.tasks.life_domains is 'All life domains this task touches (optional)';
