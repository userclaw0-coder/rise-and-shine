...
function getFallbackReasonCopy(aiStatus, aiError) {
  if (typeof aiStatus === "string" && aiStatus.startsWith("fallback:")) {
    ...
    if (!rawReason) {
      return "The planner couldn’t complete the full AI pass this time, so it set up a quick backup review instead. These suggestions are enough to keep momentum today — adjust anything you want, and apply only what feels useful. Your tasks were not changed automatically, and you can retry later if you want another pass.";
    }

    return `${humanizePlannerReason(rawReason)} Instead of forcing a shaky result through, the planner set up a quick backup review. These suggestions are enough to keep momentum today — adjust anything you want, and apply only what feels useful. Your tasks were not changed automatically, and you can retry later if you want another pass.`;
  }
...
const PHASE_CONTENT = {
  ...
  fallback: {
    ...
    hint: "The planner couldn’t finish the full AI pass cleanly this time, so it queued up enough to keep momentum today.",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
      ...
      {reviewSummary && (
        ...
        >
          ...
          <div style={{ color: "#6b7280", marginTop: 4 }}>
            {phase === "fallback"
              ? "Give them a quick scan one at a time — these are enough to keep moving today, so make one small edit if you want, then apply only what feels useful."
